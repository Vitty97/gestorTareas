...
public class GestorTareas{
    ...
    public String getTareas() {
        ...
        if(!tareas.isEmpty()){
            int ultimaPosicion = tareas.size() - 1;
            int contador = 0;
            while(contador < ultimaPosicion){
                tareasPendientes = tareasPendientes + tareas.get(contador).getDescripcion() + ", ";
                contador++;
            }
            tareasPendientes = "[" +tareasPendientes + tareas.get(contador).getDescripcion() + "]";
        }
    ...
    public void muestraTareaMasVieja() {
        if(!tareas.isEmpty()){
            System.out.println(getTareaMasVieja());
        }
    }
    ...
    public String getTareasConElTexto(String textoABuscar) {
        ...
        for(Tarea tareasConTexto : tareas) {
            if(tareasConTexto.getDescripcion().toLowerCase().contains(textoABuscar.toLowerCase())) {
                contador++;
                tareaSinTexto = tareaSinTexto + tareasConTexto.getDescripcion() + "\n";
            }
        }
    ...
    public boolean marcaComoCompletada(int posicionTarea) {
        ...
        if( posicionTarea >= 0 && posicionTarea < tareas.size()){
            if(!tareas.get(posicionTarea).getEstaCompletada()){
                tareas.get(posicionTarea).marcarComoCompletada();
                estaCompletada = tareas.get(posicionTarea).getEstaCompletada();
            }
        }
    ...
    public String getListaTareasCompletadasYNoCompletadas() {
        ...
        for (Tarea tarea : tareas) {
            if(tarea.getEstaCompletada()){
                listaTareasCompletadas += contador + ". [x] " + tarea.getDescripcion() + "\n";
            }
            else{
                listaTareasCompletadas += contador + ". [ ] " + tarea.getDescripcion() + "\n";
            }
            contador++;
        }
    ...
    public String getListaTareasCompletadasYNoCompletadasConPrioridad() {
        ...
        int contador = 1;
        for (Tarea tarea : tareas) {
            String prioridadTarea = tarea.getPrioridad() + "";
            String textoEstaCompletadaONo = "[ ] ";
            if (tarea.getEstaCompletada()) {
                textoEstaCompletadaONo = "[x] ";
            }
            listaTareasCompletadas += contador + ". " + textoEstaCompletadaONo + tarea.getDescripcion() + " " + prioridadTarea + "\n";
            contador++;
        }
        return listaTareasCompletadas;
    } 
     ...
     /**
     * 33. Cambia la prioridad de la tarea que ocupa la posicion indicada un el listado del
    ...
    public void setPrioridad(int posicionTarea, int prioridad){
        if (prioridad <= 5 && prioridad >= 1 && posicionTarea <= tareas.size()){
            tareas.get(posicionTarea - 1).setCambiarPrioridad(prioridad);
        }
    }
    ...
    public String getTareaMasPrioritaria() {
        ...
        int prioridadMaxima = 5;
        for(Tarea tareaActual : tareas){           
            if(tareaActual.getPrioridad() <= prioridadMaxima && prioridadMaxima > 0 && prioridadMaxima <= 5 ){
                tareaMasPrioritaria = "[ ] " + tareaActual.getDescripcion() + " " + tareaActual.getPrioridad();
    ...
    public String getTareasPorPrioridad() {
        ...
        //Y, una vez que tengo las tareas en orden
        //recorro el ArrayList ordenador para darle el contenido a la variable
        //textoADevolver
    ...
    /**
     * 37. Fija una fecha limite para la tarea cuyo indice nos indican. El
     * ano se especifica con 4 digitos (por ejemplo, 2019)
     */
    public void fijarFechaTope(int posicion, int dia, int mes, int ano) 
    {
        tareas.get(posicion - 1).setCambiarFecha(dia, mes, ano);
    }
